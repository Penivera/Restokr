...
async def list_signups(
    ...
    query = (
        query.offset(offset)
        .limit(page_size)
        .order_by(EarlyAccessSignup.created_at.desc())
    )
...
async def export_signups(
    ...
    if not signups:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No signups found to export"
        )
...
async def get_stats(
    ...
    for role in UserRole:
        role_result = await db.execute(
            select(func.count(EarlyAccessSignup.id)).where(
                EarlyAccessSignup.role == role
            )
        )
    ...
    city_result = await db.execute(
        select(EarlyAccessSignup.city, func.count(EarlyAccessSignup.id)).group_by(
            EarlyAccessSignup.city
        )
    )
    ...
    confirmed_result = await db.execute(
        select(func.count(EarlyAccessSignup.id)).where(
            EarlyAccessSignup.email_confirmed == True
        )
    )
